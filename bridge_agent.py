#!/usr/bin/env python3
"""TEEBridge CVM agent: HTTP server exposing DstackProof and onboarding decryption."""
...
BRIDGE_ABI = json.loads('[{"inputs":[{"name":"memberId","type":"bytes32"}],"name":"getOnboarding","outputs":[{"components":[{"name":"fromMember","type":"bytes32"},{"name":"encryptedPayload","type":"bytes"}],"name":"","type":"tuple[]"}],"stateMutability":"view","type":"function"},{"inputs":[{"name":"memberId","type":"bytes32"}],"name":"isMember","outputs":[{"name":"","type":"bool"}],"stateMutability":"view","type":"function"}]')
...
print("PROOF_JSON=" + json.dumps(PROOF_JSON))
...
def check_onboarding(bridge_addr, rpc_url='https://mainnet.base.org'):
    w3 = Web3(Web3.HTTPProvider(rpc_url))
    bridge = w3.eth.contract(address=Web3.to_checksum_address(bridge_addr), abi=BRIDGE_ABI)
    msgs = bridge.functions.getOnboarding(member_id).call()
    if not msgs:
        print("No onboarding messages yet")
        return
    for msg in msgs:
        plaintext = ecies_decrypt(derived_key, msg[1])
        print(f"ONBOARDING from={msg[0].hex()} payload={plaintext.decode()}")

if __name__ == '__main__':
    import os, threading, time
    bridge_addr = os.environ.get('BRIDGE_CONTRACT')
    if bridge_addr:
        def poll_onboarding():
            time.sleep(5)
            while True:
                try:
                    check_onboarding(bridge_addr)
                except Exception as e:
                    print(f"Onboarding check error: {e}")
                time.sleep(60)
        threading.Thread(target=poll_onboarding, daemon=True).start()
